...
if [ "${1:0:1}" = '-' ]; then
    ...
    rs0)    set -- mongod  --dbpath /mnt/data/db --port=27017 --replSet=rs0;;
    shard0) set -- mongod  --dbpath /mnt/data/db --port=27018 --shardsvr --replSet=rs0;;
    shard1) set -- mongod  --dbpath /mnt/data/db --port=27020 --shardsvr --replSet=rs1;;
    mongos) set -- mongos  --configdb configReplSet/shard0.dev.lh:27019,shard1.dev.lh:27019,shard2.dev.lh:27019 ;;
    1337)   set -- mongod  --dbpath /mnt/data/db --port=1337 ;;
